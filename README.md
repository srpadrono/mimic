...
950 tests, counted as `@Test` and `func test` declarations — a parameterized case runs more than
...
|-------|-------|---------------|
| Domain, persistence, engine, control plane, import, CLI | 618 | Linux or macOS — `swift test` |
| Design system | 52 | macOS — needs SwiftUI |
| App and coordination | 240 | macOS — hosted by the app |
| macOS UI (XCUITest) | 40 | macOS, interactive session |

The portable 618 break down as Domain 202, SpecImport 119, MimicCLICore 117, MockServerEngine 65,
Persistence 65, ControlPlane 50. The app's 240 are the six folders `MimicTests` builds —
`WorkspaceFeatureTests` 92, `MimicTests` 111, `JourneyFeatureTests` 14, `ImportFeatureTests` 13,
...
```bash
swift test                    # the portable 618, no Xcode needed
