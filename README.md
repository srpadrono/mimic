...
1282 tests, counted as `@Test` and `func test` declarations — a parameterized case runs many times
and is still one declaration. Swift Testing for units, XCTest with page objects for UI.

| Suite | Count | Where it runs |
|-------|-------|---------------|
| Domain, persistence, engine, control plane, import, CLI | 716 | Linux or macOS — `swift test` |
...
The portable 716 break down as Domain 290, SpecImport 128, MimicCLICore 113, MockServerEngine 70,
Persistence 93, ControlPlane 22. The app's 328 are the six folders `MimicTests` builds —
...
```bash
swift test                             # the portable 716, no Xcode needed
