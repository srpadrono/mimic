...
[![Tests](https://img.shields.io/badge/tests-1320%20passing-brightgreen)](#testing)
...
1320 tests, counted as `@Test` and `func test` declarations — a parameterized case runs many times
and is still one declaration. Swift Testing for units, XCTest with page objects for UI.

| Suite | Count | Where it runs |
|-------|-------|---------------|
| Domain, persistence, engine, control plane, import, CLI | 734 | Linux or macOS — `swift test` |
| Design system | 72 | macOS — needs SwiftUI |
| App and coordination | 335 | macOS — hosted by the app |
| macOS UI (XCUITest) | 179 | macOS, interactive session |

The portable 734 break down as Domain 301, SpecImport 128, MimicCLICore 113, MockServerEngine 76,
Persistence 94, ControlPlane 22. The app's 335 are the six folders `MimicTests` builds —
...
```bash
swift test                             # the portable 734, no Xcode needed
