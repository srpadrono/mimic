...
[![Tests](https://img.shields.io/badge/tests-1318%20passing-brightgreen)](#testing)
...
1318 tests, counted as `@Test` and `func test` declarations — a parameterized case runs many times
and is still one declaration. Swift Testing for units, XCTest with page objects for UI.

| Suite | Count | Where it runs |
|-------|-------|---------------|
| Domain, persistence, engine, control plane, import, CLI | 731 | Linux or macOS — `swift test` |
| Design system | 72 | macOS — needs SwiftUI |
| App and coordination | 341 | macOS — hosted by the app |
| macOS UI (XCUITest) | 174 | macOS, interactive session |

The portable 731 break down as Domain 299, SpecImport 128, MimicCLICore 113, MockServerEngine 75,
Persistence 94, ControlPlane 22. The app's 341 are the six folders `MimicTests` builds —
`WorkspaceFeatureTests` 116, `MimicTests` 181, `JourneyFeatureTests` 14, `ImportFeatureTests` 15,
...
```bash
swift test                             # the portable 731, no Xcode needed
