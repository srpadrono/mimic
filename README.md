...
[![Tests](https://img.shields.io/badge/tests-1332%20passing-brightgreen)](#testing)
...
- **Built to be driven.** A deterministic CLI (JSON out, meaningful exit codes) and a
  token-authenticated loopback HTTP control API expose 51 operations — every project, server,
...
1332 tests, counted as `@Test` and `func test` declarations — a parameterized case runs many times
and is still one declaration. Swift Testing for units, XCTest with page objects for UI.

| Suite | Count | Where it runs |
|-------|-------|---------------|
| Domain, persistence, engine, control plane, import, CLI | 739 | Linux or macOS — `swift test` |
| Design system | 73 | macOS — needs SwiftUI |
| App and coordination | 336 | macOS — hosted by the app |
| macOS UI (XCUITest) | 184 | macOS, interactive session |

The portable 739 break down as Domain 303, SpecImport 128, MimicCLICore 114, MockServerEngine 76,
Persistence 96, ControlPlane 22. The app's 336 are the six folders `MimicTests` builds —
`WorkspaceFeatureTests` 110, `MimicTests` 181, `JourneyFeatureTests` 15, `ImportFeatureTests` 15,
...
```bash
swift test                             # the portable 739, no Xcode needed
