...
public enum CommandPalette {
    ...
    ///
    /// Matching is a subsequence test, not a prefix test, so `scr` finds "Scenario: create" and
    /// `jadd` finds "Journey: add template". That is the behaviour anyone who has used Xcode's
...
public struct CommandPaletteEntry: Equatable, Hashable, Sendable, Identifiable {
    ...
    /// The catalog name spelled for a human — `journeyAddTemplate` becomes
    /// "Journey: add template".
    ///
