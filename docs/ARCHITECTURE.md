...
- Engine configuration, routes, project identity, and the journey cursor are read from one ordered snapshot for each request. The log stream is lossless; producers wait when 32 pending logs have not yet been processed. The runtime separately caps visible history at 1,000 entries.
