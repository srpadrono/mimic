...
- **DesignSystem** holds `DS*` SwiftUI tokens and components. Its shared JSON scanner bounds display formatting so a captured body cannot expand without limit; the editor enables Format only when valid JSON can be reflowed within that same output budget. Native text editors preserve undo and UTF-16 selection within a document and reset undo when document identity changes. Callers must update the identity together with the draft body it identifies.
