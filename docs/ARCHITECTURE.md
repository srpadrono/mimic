...
- Engine configuration, routes, project identity, and the journey cursor are read from one ordered snapshot for each request. The log stream is lossless; producers wait when 32 log slots are in use. Proxy requests reserve a slot before contacting the upstream or accumulating a response preview, and the runtime returns it after automatic capture. The runtime separately caps visible history at 1,000 entries. Vapor collects each incoming request body, up to 10 MiB, before the route handler; that memory is outside the log-slot bound.
