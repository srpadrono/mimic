...
| `EditorHistoryButton` identifier | — | `editor.navigation.back` / `.forward` — `CenterPaneNavigation.swift:96,104,138` | identifier | may-break | **New, no coverage.** Replaces `breadcrumb.back` / `.forward`. Mounted in both `EndpointEditorView.swift:222` and `JourneyEditorView.swift:97`. #25's criterion "back/forward still work" has no XCUITest. Unverified whether the identifier survives inside `endpointEditor`'s `.contain` container. |
| `editor.jump.<section>.<uuid>` | — | `CenterPaneNavigation.swift:173` | identifier | may-break | **New, no coverage.** Replaces `breadcrumb.option.<uuid>`. Menu items keep their own identity, but #25 says target by label; options carry `"<title>, selected"` when current. |
| `journeyEditor.jumpMenu` | — | `JourneyEditorView.swift:219` | identifier | safe | **New, no coverage.** #25 requires "jump to another journey via overflow" as an XCUITest targeting leaves by label — not written. |
