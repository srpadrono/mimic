...
`endpoint duplicate` copies the scenarios to a reachable child route (for example, `/account-summary/copy`, then `/account-summary/copy-2` if needed) and reports that route.
